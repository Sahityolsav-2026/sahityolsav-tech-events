...
export type Phase = 'Registration' | 'Idea declaration' | 'Development' | 'Event closed';
...
export function eventPhase(settings: EventSettings, now = new Date()): { phase: Phase; deadline: string | null } {
  ...
  if (time < Date.parse(settings.submission_deadline)) return { phase: 'Development', deadline: settings.submission_deadline };
