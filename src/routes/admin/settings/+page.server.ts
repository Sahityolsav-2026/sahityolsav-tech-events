...
export const actions: Actions = {
  default: async ({ locals, platform, request }) => {
    ...
    const values = {
      ...
      idea_deadline: text(form, 'idea_deadline'), submission_deadline: text(form, 'submission_deadline')
    };
    ...
    if (!(times[0] < times[1] && times[1] < times[2])) return fail(400, { message: 'Deadlines must be in chronological order.', values });
    await getDb(platform).prepare(`UPDATE event_settings SET event_name=?, timezone=?, registration_deadline=?, idea_deadline=?,
      submission_deadline=?, updated_at=? WHERE id=1`)
